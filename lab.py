...
def dijkstra(Adj, s):   # From how I learned it in Algorithms
    not_used_nodes = set()
    d = {}   # dictionary keyed with nodes mapped to their shortest distance value to the source node s
    parent = {}   # stores each node's parent, node mapped to parent node
    for v in Adj:
        d[v] = float('inf')
        parent[v] = None
...
def find_path(parents, start, end): # helper path finding function for get paths functions
    if parents[end] == None: # unreachable
...
class Graph(ABC):
    ...
    def nodes(self):
        ...
        """
        return self.nodess # I just maintain this attribute for efficiency
    ...
    def get_path_length(self, start, end):
        ...
        return distances[end] # return shortest length from start to end node
    ...
    def get_path(self, start, end):
        ...
        Adj = self.make_dijkstra_ready()
        distances, parents = dijkstra(Adj, start)
        return find_path(parents, start, end)
    ...
    def get_all_path_lengths(self):
        ...
        for node in self.nodes():
            ...
            for other_node in distances: # cycles through every path in graph
    ...
    def get_all_paths(self):
        ...
        paths = {}
        for node in self.nodes():
            distances, parents = dijkstra(Adj, node) # only runs dijkstras once to get all values from a given starting node
            for other_node in distances: # cycles through each node and finds each path 
                path = find_path(parents, node, other_node)
                if path != None:  # unreachable
...
class AdjacencyDictGraph(Graph):
    ...
    def make_dijkstra_ready(self):
        return self.graph # already in desired format for dijkstras
...
class AdjacencyMatrixGraph(Graph):
    ...
    def __init__(self):
        """Create an empty graph."""
        self.graph = []  # 2D array representation
        self.node_to_indice = {} # has all nodes mapped to their index in the 2D array
        self.indice_to_node = {} # has all indices mapped to the node their in the 2D array
        self.nodess = set()  # holds all nodes in graph
    ...
    def get_node(self, node): # helper function
        if node in self.node_to_indice: # checks to make sure node exists
            index = self.node_to_indice[node] # already exists, got its index in the matrix
            return index # returns node's index in the matrix
        else: 
            raise LookupError # the given node doesn't exist
    ...
    def make_dijkstra_ready(self): # gets it in adjacency format for dijkstra's
        Adj = {} # my dijkstras func expects an adjacency list so we will provide the desired input
        for node in self.nodes():
...
def get_most_central_node(graph):
    ...
    for n in amount_of_round_trips:
        if amount_of_round_trips[n] != possible_trips: # if node doesn't have a path to/cannot reach every node it can't be our central node
            del total_length_round_trips[n]  # don't include it in our eval of lowest average = most central node
